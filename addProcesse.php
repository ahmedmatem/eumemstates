...
$query = "SELECT * FROM countries WHERE name='" . mysqli_real_escape_string($con, $country) . "'";
...
if($_FILES['flagFile']['error'] > 0){
    ...
    if((($_FILES['flagFile']['type'] == 'image/gif')
        ...
        $filename = strtolower($country) . '.' . $extension;
...
$stmt = mysqli_prepare($con, "INSERT INTO countries(name, capital, population, state, flag) VALUES(?, ?, ?, ?, ?)");
